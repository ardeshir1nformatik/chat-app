...
io.on('connection', (socket) => {
    ...
    // socket.on('increment', () => {
    //     count++
    //     //socket.emit('countUpdated', count)    // this emits the event to a particular/single connection
    //     io.emit('countUpdated', count)  // this emits the event to all connections
    // })
    ...
    socket.on('join', ({ username, room }, callback) => {
        ...
        socket.emit('sendMessage', generateMessage('Admin', `Welcome ${user.username}`))
    ...
    socket.on('theMessage', (text, callback) => {
        ...
        io.to(user.room).emit('sendMessage', generateMessage(user.username, text))
        callback()
    })
